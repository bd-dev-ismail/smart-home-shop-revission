...
const Cart = () => {
  ...
  const handalRemoveItem = (id) =>{
    ...
    setCart(remaning);
    removeFromDb(id);
    toast.warning('Remove Successful' ,{autoClose: 500});
  }
  let total = 0;
  for(const product of cart){
    total = total + product.quantity * product.price;
  }
  const handalDeleteAll = ()=>{
    setCart([]);
    deleteShoppingCart();
    toast.error('Delete All Cart', {autoClose: 500});
  }
  return (
    <div className='flex min-h-screen items-start justify-center bg-gray-100 text-gray-900'>
      <div className='flex flex-col max-w-3xl p-6 space-y-4 sm:p-10 '>
        ...
        <ul className='flex flex-col divide-y divide-gray-700'>
          {
            cart.map((product, index) => <CartItem product={product} key={index} handalRemoveItem={handalRemoveItem}></CartItem>)
          }
        </ul>
